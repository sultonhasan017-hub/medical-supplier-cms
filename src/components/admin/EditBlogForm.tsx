...
import { updateBlogAction } from '../../app/[locale]/admin/kelola-blog/[id]/edit/actions';
...
export default function EditBlogForm({ article }: { article: Article }) {
  const router = useRouter();
  const [error, setError] = useState('');
  const [loading, setLoading] = useState(false);
  const [activeTab, setActiveTab] = useState<'ID' | 'EN'>('ID');
  const [metaDescId, setMetaDescId] = useState(article.metaDesc_id);
  const [metaDescEn, setMetaDescEn] = useState(article.metaDesc_en);
  ...
  const [additionalImages, setAdditionalImages] = useState<string[]>(article.additionalImages || []);
  ...
  const handleThumbnailUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (data.success) {
        ...
        toast.success('Thumbnail artikel berhasil diganti');
      } else {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const res = await updateBlogAction(article.id, formData);
      if (res?.error) {
        setError(res.error);
  ...
  return (
    <div className="max-w-4xl space-y-6 pb-24">
      <div>
        <h1 className="text-2xl font-bold text-[var(--color-dark-navy)]">Edit Artikel</h1>
        <p className="text-sm text-slate-500">
          Perbarui konten artikel: <span className="font-medium text-[var(--color-primary-blue)]">{article.title_id}</span>
        </p>
      </div>
          ...
          <Card>
            <CardContent className="p-6 space-y-4">
              ...
              <div className="space-y-2">
                ...
                <input
                  name="thumbnailAlt"
                  defaultValue={article.thumbnailAlt || ''}
                  className="w-full rounded-md border border-[var(--color-airy-blue)] px-3 py-2 text-sm focus:border-[var(--color-primary-blue)] focus:outline-none focus:ring-1 focus:ring-[var(--color-primary-blue)]"
            ...
            <CardContent className="p-6 space-y-4">
              <div className="space-y-2">
                ...
                <input
                  value={article.slug}
                  readOnly
                  className="w-full rounded-md border border-slate-200 bg-slate-50 px-3 py-2 text-sm text-slate-400 cursor-not-allowed"
                />
                <p className="text-xs text-slate-500">Slug tidak bisa diubah setelah artikel dibuat.</p>
              </div>

              <div className="space-y-2">
                <label className="text-sm font-semibold text-[var(--color-dark-navy)]">Status Publikasi</label>
                <div
                ...
                >
                  <div className={`h-4 w-4 rounded-full border-2 flex items-center justify-center ${
                    isPublished ? 'border-emerald-500 bg-emerald-500' : 'border-slate-300'
                  }`}>
                    {isPublished && <div className="h-2 w-2 rounded-full bg-white" />}
                  </div>
                  <span className={`text-sm font-medium ${isPublished ? 'text-emerald-700' : 'text-slate-500'}`}>
                    {isPublished ? 'Published — Terlihat Publik' : 'Draft — Tidak Terlihat'}
                  </span>
                </div>
              </div>
            </CardContent>
